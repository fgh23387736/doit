<?php
	...
	*/
	...
	function toDelete($request_data){
		...
		$sql_f="DELETE FROM `province` WHERE ";
		$sql_b="";
		$wherevalues=array();
	...
	function toGet($request_data){
		...
		$sql_f="SELECT * FROM `province` WHERE ";
		$sql_c="SELECT count(*) AS `ALLNumber` FROM `province` WHERE ";
		$sql_b="";
		$wherevalues=array();
		switch($request_data['Type']){
			case '0':{
			...
			case '1':{
				if(iseset($request_data['Search']['Name'])&&!empty($request_data['Search']['Name']))
				{
