<?php
	...
	function toGet($request_data){
		...
		$sql_f="SELECT * FROM `city` WHERE ";
		$sql_c="SELECT count(*) AS `ALLNumber` FROM `city` WHERE ";
		$sql_b="";
		$wherevalues=array();
		...
		switch($request_data['Type'])
		{
			...
			case '1':{
				...
				{
					$Name=explode('+',$request_data['Search']['Name']);
					$sql_b="`Id` IN (";
